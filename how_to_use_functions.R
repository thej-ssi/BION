...
setwd("")
setwd("S:/MPV/Thor Bech Johannesen/DFPT")
setwd("H:/SFSDI/BPSAFD/MPV/THEJ/R_crash_course")
setwd("P:/MPV/THEJ/R_crash_course")
...
### have a look at the first 2 lineS of the metadata:
sample_data(po)[1:2,]
...
Alphadiv_plot = make_alphadiversity_object(po,variable_name = "Group",plot_title = "Alphadiversity of control and treatment samples",color_vector[c(1,3)])
Alphadiv_plot
...
#### It runs a lot faster without doing the anosim test ####
PCoA_plot = make_PCoA_object(po,variable_name = "Group",plot_title = "PCoA plot of control and treatment samples",color_vector[c(1,3)],perform_anosim = FALSE)
PCoA_plot
...
taxa_comparison_df[which(as.numeric(as.vector(taxa_comparison_df$`p adjusted`))<0.05),]


### Try and print it to a file

write.table(taxa_comparison_df,file = "taxa_comparison.txt",sep = "\t", row.names = F)
...
analysis_list = run_cross_sectional_analysis(po,"D.fragilis",RColorBrewer::brewer.pal(9,"Set1")[c(1,3)])
# 1. Alphadiversity               analysis_list[[1]]
# 2. PCoA                         analysis_list[[2]]
# 3. Bar plot                     analysis_list[[3]]
# 4. Heatmap                      analysis_list[[4]]
# 5. taxa_comparison              analysis_list[[5]]
