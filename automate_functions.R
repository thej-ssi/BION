...
load_data <- function(input_file) {
  if (tolower(substr(input_file,nchar(input_file)-4,nchar(input_file))) == ".xlsx") {
    ...
    tsv_input = read.table(input_file,sep = '\t', comment.char = "", header = TRUE, stringsAsFactors = FALSE)
...
load_metadata <- function(metadata_file, metadata_split_variable) {
  if (metadata_file == "" | is.na(metadata_file)) {
    ...
    if (tolower(substr(metadata_file,nchar(metadata_file)-4,nchar(metadata_file))) == ".xlsx") {
      tsv_input = read.xlsx(metadata_file, 1) # read the first sheet
    }
    else {
...
setup_phylo_object_2 <- function(tsv_input,metadata) {
  ...
  tax_table = as.matrix(tsv_input[,1:7])
  tax_table = trimws(tax_table)
  tax_prokaryot = tax_table[tax_table[,1]=="d__Bacteria"|tax_table[,1]=="d__Archaea",]
  otu_prokaryot = otu_table[tax_table[,1]=="d__Bacteria"|tax_table[,1]=="d__Archaea",]
  tax_eukaryot = tax_table[tax_table[,1]=="d__Eukaryota" & !tax_table[,3]=="c__Mammalia",]
  otu_eukaryot = otu_table[tax_table[,1]=="d__Eukaryota" & !tax_table[,3]=="c__Mammalia",]
  tax_ranks = c("Domain","Phylum","Class","Order","Family","Genus","Species")
  rownames(tax_prokaryot) <- paste0("OTU", 1:nrow(tax_prokaryot))
  rownames(otu_prokaryot) = rownames(tax_prokaryot)
  rownames(tax_eukaryot) <- paste0("OTU", 1:nrow(tax_eukaryot))
...
make_abundance_barplot <- function(po,top10_species,plot_name) {
...
alphadiversity_plot <- function(po,plot_name) {
  r <- data.frame(ID=sample_names(po), type=factor(get_variable(po)$sample_groups), richness=colSums(otu_table(po) > 0), estimate_richness(po))
...
test_tax = as.data.frame(tax_table(po_eukaryot))
...
run_all <- function(rare_prokaryot,rare_eukaryot,rare_fungi,summary_list) {
  ...
  ### PCoA plots
  PCoA_prokaryot<-make_PCOA_plot(rare_prokaryot,'PCoA of prokaryotic microbiome based on Bray-curtis dissimilarity')
  ggsave(filename = paste0(output_dir,"/Fig_2-1_PCoA_prokaryot.png"),plot = PCoA_prokaryot[[1]])
  
  PCoA_eukaryot<-make_PCOA_plot(rare_eukaryot,'PCoA of eukaryotic microbiome based on Bray-curtis dissimilarity')
  ggsave(filename = paste0(output_dir,"/Fig_2-2_PCoA_eukaryot.png"),plot = PCoA_eukaryot[[1]])
  
  PCoA_fungi<-make_PCOA_plot(rare_fungi,'PCoA of fungal microbiome based on Bray-curtis dissimilarity')
  ...
  write.xlsx(genus_comparison_prokaryot,file = filename,row.names = FALSE,sheetName = "Prokaryot")
  write.xlsx(genus_comparison_eukaryot,file = filename,row.names = FALSE,append=TRUE,sheetName = "Eukaryot")
  write.xlsx(genus_comparison_fungi,file = filename,row.names = FALSE,append=TRUE,sheetName = "Fungi")
  ...
  write.xlsx(rare_matrix,file = summary_excel_file,row.names = FALSE,sheetName = "Rarefaction stats")
  write.xlsx(alpha_matrix,file = summary_excel_file,row.names = FALSE,append=TRUE,sheetName = "Alphadiversity p-values")
  write.xlsx(ANOSIM_matrix,file = summary_excel_file,row.names = FALSE,col.names = FALSE,append=TRUE,sheetName = "ANOSIM stats")
